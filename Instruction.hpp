...
struct InstFormatI: Instruction, InstFormat {
    ...
    auto dump() -> void {
        // $rd, $rs1, $imm12
        AlignedPrintf<DumpOptions::ArgstrAlign>("%s, %s, %d", regname[rd], regname[rs1], imm12);
        Instruction::dump();
    }
};
...
struct InstFormatS: Instruction, InstFormat {
    ...
    auto dump() -> void {
        // $rs2, $imm12($rs1)
        AlignedPrintf<DumpOptions::ArgstrAlign>("%s, %d(%s)", regname[rs2], imm12, regname[rs1]);
        Instruction::dump();
    }
};
...
struct InstFormatB: Instruction, InstFormat {
    ...
    auto dump() -> void {
        // $rs1, $rs2, $imm13
        AlignedPrintf<DumpOptions::ArgstrAlign>("%s, %s, %d", regname[rs1], regname[rs2], imm13);
        Instruction::dump();
    }
};
...
struct InstFormatU: Instruction, InstFormat {
    ...
    auto dump() -> void {
        // $rd, $imm20
        AlignedPrintf<DumpOptions::ArgstrAlign>("%s, %d", regname[rd], imm);
        Instruction::dump();
    }
};
...
struct InstFormatJ: Instruction, InstFormat {
    ...
    auto dump() -> void {
        // $rd, $imm21
        AlignedPrintf<DumpOptions::ArgstrAlign>("%s, %d", regname[rd], imm21);
        Instruction::dump();
    }
};
...
template <> inline auto ADDI  ::Execute() -> void { rdv = rs1v + imm12; }
template <> inline auto SLTI  ::Execute() -> void { rdv = (cast<i32>(rs1v) < cast<i32>(imm12)) ? 1 : 0; }
template <> inline auto SLTIU ::Execute() -> void { rdv = (rs1v < imm12) ? 1 : 0; }
template <> inline auto ANDI  ::Execute() -> void { rdv = rs1v & imm12; }
template <> inline auto ORI   ::Execute() -> void { rdv = rs1v | imm12; }
template <> inline auto XORI  ::Execute() -> void { rdv = rs1v ^ imm12; }
...
template <> inline auto JAL   ::Execute() -> void { rdv = pc + 4; pcv = pc + imm21; }
template <> inline auto JALR  ::Execute() -> void { rdv = pc + 4; /*pcv = rs1v + imm12;*/ } // TODO: add pcv field for JALR
template <> inline auto BEQ   ::Execute() -> void { pcv = pc + (rs1v == rs2v ? imm13 : 0); }
template <> inline auto BNE   ::Execute() -> void { pcv = pc + (rs1v != rs2v ? imm13 : 0); }
template <> inline auto BLT   ::Execute() -> void { pcv = pc + ((cast<i32>(rs1v) < cast<i32>(rs2v)) ? imm13 : 0); }
template <> inline auto BLTU  ::Execute() -> void { pcv = pc + ((rs1v < rs2v) ? imm13 : 0); }
template <> inline auto BGE   ::Execute() -> void { pcv = pc + ((cast<i32>(rs1v) > cast<i32>(rs2v)) ? imm13 : 0); }
template <> inline auto BGEU  ::Execute() -> void { pcv = pc + ((rs1v > rs2v) ? imm13 : 0); }
template <> inline auto LB    ::Execute() -> void { rdv = rs1v + imm12; }
template <> inline auto LH    ::Execute() -> void { rdv = rs1v + imm12; }
template <> inline auto LW    ::Execute() -> void { rdv = rs1v + imm12; }
template <> inline auto LBU   ::Execute() -> void { rdv = rs1v + imm12; }
template <> inline auto LHU   ::Execute() -> void { rdv = rs1v + imm12; }
template <> inline auto SB    ::Execute() -> void { addr = rs1v + imm12; }
template <> inline auto SH    ::Execute() -> void { addr = rs1v + imm12; }
template <> inline auto SW    ::Execute() -> void { addr = rs1v + imm12; }
...
template <> inline auto JALR  ::WriteBack(u32 &pc, u32 reg[32]) -> void { if (rd != 0) reg[rd] = rdv; pc = /*pcv*/ rs1v + imm12; }
